...
@lru_cache(maxsize=1)
def obterExtensaoPostgres():
    return ConfiguracaoAtual.obterExtensaoPostgres()


@lru_cache(maxsize=1)
def obterExtensaoSqlServer():
    return ConfiguracaoAtual.obterExtensaoSqlServer()


def _configurarEngine(engine: Engine) -> Engine:
    engine.echo = ConfiguracaoAtual.mostrarLogsDb
    return engine


@lru_cache(maxsize=1)
def obterEnginePostgres() -> Engine:
    extensaoBanco = obterExtensaoPostgres()
    if extensaoBanco is not None:
        return _configurarEngine(extensaoBanco.obter_engine())
...
def obterEngineSqlServer() -> Engine | None:
    extensaoBanco = obterExtensaoSqlServer()
    if extensaoBanco is not None:
        return _configurarEngine(extensaoBanco.obter_engine())
...
def obterSessaoPostgres() -> Session:
    extensaoBanco = obterExtensaoPostgres()
    if extensaoBanco is not None:
        return extensaoBanco.obter_sessao_simples()
