...
def PopularSessaoUsuario(dadosUsuarioApi: dict[str, Any] | None) -> bool:
    ...
    permissoes_ativas = [
        chave for chave, permitido in session["permissions"].items() if permitido
    ]
    ...
    logger.info(
        "Sessao autenticada: user_id=%s login=%s grupo=%s permissoes_ativas=%s total=%s",
        session.get("user_id"),
        session.get("user_name"),
        session.get("user_group", {}).get("group_code"),
        permissoes_ativas[:10],
        len(permissoes_ativas),
    )
