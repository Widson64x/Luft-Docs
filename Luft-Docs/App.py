...
try:
    ...
    if erro.name == "Config":
        print(
            "ERRO CRITICO: Nao foi possivel encontrar o arquivo Config.py em "
            f"{APP_DIR}"
        )
    else:
        print(
            "ERRO CRITICO: Falha ao importar Config.py por dependencia ausente: "
            f"{erro.name}"
        )
    raise SystemExit(1) from erro
except ImportError as erro:
    print(f"ERRO CRITICO: Falha ao carregar Config.py: {erro}")
    raise SystemExit(1) from erro
...
def InicializarLuftCore(aplicacaoFlask: Flask) -> LuftCorePackages:
    ...
    endpointMensagens = f"{cfg.BASE_PREFIX}/_luftcore/mensagens".replace("//", "/")
    aplicacaoFlask.config["LUFT_ENDPOINT_MENSAGENS"] = endpointMensagens
    aplicacaoFlask.config["LUFT_MESSAGE_BACKEND_ENDPOINT"] = endpointMensagens
    aplicacaoFlask.jinja_env.globals["luft_endpoint_mensagens"] = endpointMensagens
    aplicacaoFlask.jinja_env.globals["luft_message_backend_endpoint"] = endpointMensagens
...
# SQL Server (permissoes + usuarios) — opcional; desativado sem Vault/SQL configurado
if cfg.SQLSERVER_URL:
    ...
    logger.warning(
        "SQL Server nao configurado. "
        "Permissoes e busca de usuarios no AD estarao indisponiveis."
    )
    ...
    try:
        with app.app_context():
            ...
            if engineSqlServer is not None:
                # Criar tabelas SQL Server usando o modelo definido em Models.Permissoes
                #BaseSqlServer.metadata.create_all(bind=engineSqlServer)
                pass
        logger.info("Verificação automática de tabelas SQL Server desativada.")
    except Exception as _e_createall:
        logger.warning("Erro ao inicializar engine SQL Server: %s", _e_createall)

# ---------------------------------------------------------------------------
# flask_login
# ---------------------------------------------------------------------------
login_manager = LoginManager(app)
login_manager.login_view = "Inicio.exibirInicio"
...
def InicializarBancoDados():
    """
    Comando CLI responsavel por inicializar as estruturas fisicas do banco de dados (APENAS POSTGRES).
    """
    click.echo("Processando a inicializacao de tabelas no banco de dados PostgreSQL...")
    BasePostgres.metadata.create_all(bind=obterEnginePostgres())

    # AVISO: BaseSqlServer.metadata.create_all foi removido. 
    # As tabelas do SQL Server (intec.dbo.*) são apenas leitura estrutural na aplicação 
    # e são gerenciadas unicamente pelo DBA.
...
if __name__ == "__main__":
    ...
    if modoDepuracao:
        ...
        logger.info(
            "Iniciando servidor Waitress local em http://127.0.0.1:%s%s",
            portaExecucao,
            cfg.BASE_PREFIX,
        )
        serve(
