...
class ServicoEditor:
    ...
    def _listarSubmodulosGlobais(self) -> list[dict[str, str]]:
        """Retorna os submodulos globais com dados reutilizaveis para tela e AJAX."""
        ...
        for arquivo in diretorio_global.rglob("*.md"):
            caminho_relativo = arquivo.relative_to(diretorio_global).as_posix()
            nome_referencia = arquivo.stem.replace("_", " ").strip()
            chave_nome = nome_referencia.casefold()
            ocorrencias_por_nome[chave_nome] = ocorrencias_por_nome.get(chave_nome, 0) + 1
        ...
        for item in submodulos:
            nome = item["name"]
            if ocorrencias_por_nome.get(nome.casefold(), 0) > 1:
                item["label"] = f'{nome} ({item["path"]})'
            else:
                item["label"] = nome
    ...
    def obterRespostaListagemSubmodulos(self) -> dict[str, Any]:
        ...
        diretorio_global = Path(DATA_ROOT) / "global"
        diretorio_global.mkdir(exist_ok=True)
        submodulos = self._listarSubmodulosGlobais()

        if (
            request.headers.get("X-Requested-With") == "XMLHttpRequest"
            or request.args.get("formato") == "json"
        ):
            return self._respostaJson(
                submodules=[
                    {
    ...
    def obterRespostaPreviewMarkdown(self) -> dict[str, Any]:
        """Renderiza um preview de markdown com o mesmo pipeline da visualizacao final."""
        if not self._usuarioPossuiPermissao(ChavesPermissao.EDITAR_MODULOS):
            return self._respostaJsonErro("Acesso negado.", 403)

        payload = request.get_json(silent=True) or {}
        conteudo_markdown = self._limparLinhasEmBranco(str(payload.get("markdown", "")))
        modulos, palavras_globais = CarregarModulos()
        html = ConverterWikiLinks(conteudo_markdown, modulos, palavras_globais)
        return self._respostaJson(
            html=self._normalizarHtmlRenderizado(html, self._obterToken())
        )
    ...
    @staticmethod
    def _normalizarHtmlRenderizado(html: str, token: str) -> str:
        if not html:
            return ""

        conteudo_corrigido = html.replace("__TOKEN_PLACEHOLDER__", token or "")
        prefixo_legado_download = 'href="/downloads/docs/'
        prefixo_download_atual = (
            f'href="{url_for("Arquivos.baixarPelaRaiz", token=token or "")}&download='
        )
        return conteudo_corrigido.replace(
            prefixo_legado_download,
            prefixo_download_atual,
        )
